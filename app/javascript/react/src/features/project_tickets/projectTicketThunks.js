...
export const fetchProjectTickets = createAsyncThunk(
    ...
    async (id) => {
        ...
        try {
            const response = await axios.get(`http://192.168.1.20:3000/api/v1/projects/${id}/users_and_bugs_by_project`, {
                headers: {
        ...
        async ({ id : developer_id, projectId }) => {
            ...
            try {
                const response = await axios.post(
                    `http://192.168.1.20:3000/api/v1/projects/${projectId}/assign_user/${developer_id}`, 
                    null,
                    {
                        headers: {
                            "Content-Type": "application/json",
        ...
        async ({ project_id , id }) => {
            ...
            try {
                const response = await axios.delete(
                    `http://192.168.1.20:3000/api/v1/projects/${project_id}/remove_user/${id}`,
                    {
                        headers: {
                            "Content-Type": "application/json",
                            Authorization: token,
